...
BEGIN
    ...
    -- Obtener el valor m�ximo de legajo y sumarle 1
...
BEGIN
    ...
    DECLARE @id INT,
            ...
            @descripcion_nueva VARCHAR(50) = 'Descripci�n Nueva',
    ...
    VALUES ('Descripci�n Original', 'Original Description');
...
BEGIN
    ...
    DECLARE @id INT,
            @descripcion_original VARCHAR(50),
            @descripcion_nueva VARCHAR(50) = 'Nueva Descripci�n';

    -- Inserto dato de prueba en la tabla CATEGORIA
    INSERT INTO seguridad.CATEGORIA (descripcion, es_valido)
    VALUES ('Descripci�n Original', 1);

    -- Obtener el ID de la categor�a insertada
    SELECT @id = SCOPE_IDENTITY();
    ...
    -- Mostrar la categor�a original
    SELECT * FROM seguridad.CATEGORIA WHERE id = @id;

    -- Ejecutar el procedimiento de actualización (modificar solo la descripci�n)
    ...
    -- Verificar si la descripci�n se ha actualizado
    IF (@descripcion_original != (SELECT descripcion FROM seguridad.CATEGORIA WHERE id = @id))
    BEGIN
        PRINT 'TEST PASADO - Actualicaci�n de CATEGOR�A exitosa.';
    END
    ELSE
    BEGIN
        PRINT 'TEST FALLIDO - Error en la actualización de la descripci�n.';
    END;
...
BEGIN
    ...
    -- Almacenar el valor original del tel�fono
    ...
    -- Verificar si el tel�fono ha sido actualizado
...
BEGIN
	...
	BEGIN CATCH
		PRINT 'Error en la inserci�n de factura';
	END CATCH
    ...
    BEGIN CATCH
        -- En caso de error, revertir la transacci�n y mostrar un mensaje de error
...
BEGIN
    ...
    DECLARE @resultadoEstado BIT,
	 ...
	 BEGIN TRY
		...
		ELSE 
			PRINT 'TEST FALLIDO - Error en la actualización de nota de cr�dito';
		

	END TRY

	BEGIN CATCH
		PRINT 'ERROR EN EL TEST - Error inesperado en la insercion de los datos o ejecuci�n del procedure';
	END CATCH
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Ejecutar el procedimiento de actualización para cambiar el g�nero a 'male'
        EXEC actualizaciones.ActualizarCliente @id = @id_cliente, @genero = @nuevo_genero;

        -- Verificar que el g�nero se haya actualizado correctamente y que id_tipo se mantenga igual
...
BEGIN
    ...
    -- Verificar si la actualización fue exitosa en el campo 'provincia' y los dem�s campos no fueron modificados
    IF EXISTS (
...
BEGIN
    ...
    -- Insertar producto de prueba asociado a la categor�a
	...
	-- Verificar si la actualización fue exitosa y los dem�s campos no fueron modificados
    IF EXISTS (
...
BEGIN
    ...
    -- Mostrar el registro despu�s de la actualización
    SELECT * FROM productos.IMPORTADO WHERE id_producto = @id_producto;
...
BEGIN
    ...
    -- Insertar una categor�a de prueba
    INSERT INTO seguridad.CATEGORIA (descripcion) VALUES ('Categoria de prueba');
    SET @id_categoria = SCOPE_IDENTITY();

    -- Insertar un producto de prueba asociado a la categor�a
    INSERT INTO productos.PRODUCTO (precio_unidad, nombre_producto, id_categoria, es_valido)
    VALUES (100, 'Nombre prueba', @id_categoria, 1);
    SET @id_producto = SCOPE_IDENTITY();
    ...
    VALUES ('Credit Card', 'Tarjeta de Cr�dito');
    ...
    -- Obtener el m�ximo legajo y sumarle 1 para el nuevo empleado
    SELECT @legajo = ISNULL(MAX(legajo), 0) + 1 FROM seguridad.EMPLEADO;
    ...
    -- Mostrar el registro de VENTA despu�s de la actualización
    SELECT * FROM transacciones.VENTA WHERE id = @id_venta;
